<#
.SYNOPSIS
    Publishes MarkdownViewer to a stable per-user location and optionally registers it
...
.DESCRIPTION
    ...
    the real install goes to %LOCALAPPDATA%\Programs\MarkdownViewer.
...
[CmdletBinding()]
param(
    [string] $Destination = (Join-Path $env:LOCALAPPDATA 'Programs\MarkdownViewer'),
...
$project = Join-Path $PSScriptRoot '..\MarkdownViewer\MarkdownViewer.csproj'
...
if ($running) {
    Write-Host "Closing running MarkdownViewer (pid $($running.Id -join ', '))..."
...
$exe = Join-Path $Destination 'MarkdownViewer.exe'
